...
//
///**
// * VmAllocationPolicySimpleCan is an VmAllocationPolicy that chooses, as the host for a VM, the host
// * with less PEs available. It is therefore a Best Fit policy, allocating VMs into the
// * host with less available PE.
// *
...
// */
//
//public class VmAllocationPolicySimpleCan_ACO_3 extends VmAllocationPolicy {
//
//
...
//	 */
//
//	public VmAllocationPolicySimpleCan_ACO_3(List<? extends Host> list) {
//		super(list);
//
...
//				// select new tuple
//				selectNewTuple(tuples,antTuples,antAvailableTuples,hostUtilization,antNumberOfPMsToSleep,
//						antHostList,pheromoneList,heuristicList,migrationMapList,destinationPMs,sourcePMs,vmList);
//				// update trails
//				pheromoneUpdate(pheromoneList);
...
//							   List<ArrayList<Double>> hostUtilization,
...
//						}
//					}
//					theFunction(i,newTuple,antTuples,antAvailableTuples,antHostList,hostUtilization,antNumberOfPMsToSleep,
//							migrationMapList,destinationPMs,sourcePMs,vmList);
//				}else{
...
//		userId = newTuple[3];
//
//		Vm vmToMigrate = vmList.get(vm); //antHostList.get(i).Vm.get(vm,userId);
...
//			antHostList.get(i).get(source).vmDestroy(vmToMigrate);
//			antHostList.get(i).get(destination).vmCreate(vmToMigrate);
//			if(newSourceHostUtilizationValue == 0){
//				if(!antNumberOfPMsToSleep.get(i).contains(source))
//					antNumberOfPMsToSleep.get(i).add(source);
//			}else{
//				if(antNumberOfPMsToSleep.get(i).contains(source)){
//					int removeIndex = antNumberOfPMsToSleep.get(i).indexOf(source);
//					antNumberOfPMsToSleep.get(i).remove(removeIndex);
//				}
//			}
//		}
//
//		antTuples.get(i).add(newTuple.clone());
//		Host sourceHostMigration = sourcePMs.get(sourcePMs.indexOf(antHostList.get(i).get(newTuple[0])));
//		Host destinationHostMigration = destinationPMs.get(destinationPMs.indexOf(antHostList.get(i).get(newTuple[2])));
//		// migration plan update
//		migrationMapList.set(i,migrationUpdate(migrationMapList.get(i),sourceHostMigration,destinationHostMigration,newTuple[1],newTuple[3]));
...
//			}
//		}
//		// while remove operation index numbers may change in the list,
//		// therefore indexes are read from antTuple
//		int counter = 0;
//		for(int j=0;j<removeList.size();j++){
//			antAvailableTuples.get(i).remove(removeList.get(j)-counter);
//			counter++;
//		}
//	}
//
...
//									  List<ArrayList<Integer[]>> antTuples,
//									  List<ArrayList<antHost>> antHostList,
//									  List<ArrayList<Integer>> antNumberOfPMsToSleep){
...
//				newDestinationHostUtilizationValue = previousDestinationHostUtilizationValue + vmToMigrate.getCurrentRequestedTotalMips();
//				if(newSourceHostUtilizationValue < 0 || newDestinationHostUtilizationValue < 0 ){
//					// this utilization must be removed, it is false
//					antTuples.get(i).remove(j);
//					j--;
//				}else{
//					hostUtilization.get(i).set(source,newSourceHostUtilizationValue);
//					hostUtilization.get(i).set(destination,newDestinationHostUtilizationValue);
...
//	 */
//
//	public void pheromoneUpdate (List<Double> pheromoneList){
//		for(int i=0;i<pheromoneList.size(); i++)
...
//			heuristicList.add(heuristicCalculation(destinationPMs.get(destination),sourcePMs.get(source).getVm(vm,userId)));
//		}
