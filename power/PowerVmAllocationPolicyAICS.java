...
import static org.cloudbus.cloudsim.cloudsim_cansusam_vmallocation.power.canConstants.NSGAII_nonmigabst.*;

/**
 * @author canssm
 */
public class PowerVmAllocationPolicyAICS extends PowerVmAllocationPolicyMigrationAbstract {
	 ...
	 * @param list the list
	 */
	public PowerVmAllocationPolicyAICS(List<? extends Host> list,
	...
	public List<Map<String, Object>> optimizeAllocation(List<? extends Vm> vmList) {
		...
		while(iterationLimit > iteration){
			...
			for(int i=0; i<antNumber; i++){
				...
				for(Host h:hostList){
					...
					hostCounter++;
					if(sleepingHosts.contains(h))
						antNumberOfPMsToSleep.get(i).add(h.getId());
				}
			...
			for(int k=0; k<antVMNumber; k++){ // TODO antnumber <-> vmList.size()
				for(int l=0; l<antNumber; l++){
					...
					if (antSelectedVm[l] != -1 && hostSelectionResult != -1) {
						heuristicUpdate(vmListWithoutNullHosts,antSelectedVm[l],hostSelectionResult,
											antHostList.get(l),antHeuristicsVM.get(l),antHeuristicsHost.get(l));
					}
				}
			}
			...
			/**
			 * TODO
			 * Add a variable which keeps the type of sort will be used as a base of updates (rank|energy|sleep)
		...
		try {
		    String staticFolderName = folderName;
			String folderName =  "myexperiments/" +
					getClass().getName()
							.replace("org.cloudbus.cloudsim.cloudsim_cansusam_vmallocation.power.", "")
							.split("@")[0] + "/" + staticFolderName + "/";
			File folder = new File(folderName);
			if(!folder.exists())
				folder.mkdir();
			folderName = folderName + "IterationNumbers/";
...
//			totalRequestedMips_NOTUSED = hostList.get(i).getTotalMips()-hostList.get(i).getVmScheduler().getAvailableMips();
			...
			for (Vm vm : h.getVmList()) {
				totalRequestedMips +=  vm.getTotalUtilizationOfCpuMips(CloudSim.clock());//vm.getMips(); //vm.getCurrentRequestedTotalMips(); //
...
//				}
	...
	public <T extends Comparable> int[] sortValues(T[] array){
		int[] sortedIndicesOfVMMigration = IntStream.range(0, array.length)
				.boxed().sorted((i, j) -> array[i].compareTo(array[j]) )
				.mapToInt(ele -> ele).toArray();
		return sortedIndicesOfVMMigration;
	}
	...
	public int[] ranking(List<ArrayList<Integer>> nonDomination, double[] crowding) {
		...
		for(ArrayList<Integer> front : nonDomination) {
			...
			int[] sortedCrowdedDistanceIndices = sortValues(crowdingDistanceOfFront);
    ...
    public List<ArrayList<Integer>> nonDominatedSort(Double[] antEnergyInfo, Integer[] antVMMigrationNumber) {
		...
		for(int i=0; i<antNumber; i++) {
			...
			for(int j=0; j<antNumber; j++) {
				...
				if(dominationCheck(antEnergyInfo[i],antEnergyInfo[j],
						antVMMigrationNumber[i],antVMMigrationNumber[j]))
					solutionsDominated.get(i).add(j);
				else if(dominationCheck(antEnergyInfo[j],antEnergyInfo[i],
						antVMMigrationNumber[j],antVMMigrationNumber[i]))
	 ...
	 * @param antHostList
	 */
	public static void calculateEnergy2(Double[] antEnergyInfo, List<LinkedList<Map<String, Object>>> migrationMapList,
...
//					counter++;
//				}
//				if(counter == antVMNumber ||counter == vmList.size()) break;
//			}
//			if(counter == antVMNumber||counter == vmList.size()) break;
//		}
	...
	public int selectNewHost(Vm selectedVm,
		...
		List<Double> availableHostPheromones = new ArrayList<Double>();	// probabilities of hosts according to index
		List<Double> availableHostHeuristics = new ArrayList<Double>();	// probabilities of vms according to index
		...
		for (int i=0; i<theAntHost.Host.size(); i++) {
			...
			if (    !theAntHost.Vm.get(i).contains(selectedVm.getId())
					...
					&& !antNumberOfPMsToSleep.contains(theAntHost.Host.get(i)) // TODO turn ON/OFF
...
//					Log.print("");
				availableHostPheromones.add(pheromoneList.get(i));
				availableHostHeuristics.add(heuristicList.get(i));
			}
		}
		...
		double vmMips = selectedVm.getTotalUtilizationOfCpuMips(CloudSim.clock());//getMips();//getCurrentRequestedTotalMips();//
...
//		indexOfDestinationHost = findAvailableHostFromSortedIndexes(availableHosts,sortedHostIndexes);
		 ...
		 *  TODO change selection of max utilized host
		 */
		...
		if(indexOfDestinationHost == 566 && CloudSim.clock() > 27600.0){
			System.out.print("");
		}
		...
		int destinationID = theAntHost.Host.get(indexOfDestinationHost); // does ant host contains sleeping ones?
		if(antNumberOfPMsToSleep.contains(destinationID))
			antNumberOfPMsToSleep.remove(antNumberOfPMsToSleep.indexOf(destinationID));
...
//		double avMips = h.getVmScheduler().getAvailableMips()-(hostUtilization-h.getTotalMips());
        ...
        double vmMips = vm.getTotalUtilizationOfCpuMips(CloudSim.clock());//vm.getMips();//vm.getCurrentRequestedTotalMips();//
		return (h.getVmScheduler().getPeCapacity() >= vmMips // TODO 0.9 keeps efficiency low, but prevents utilization crash
				&& newAvailableMips*0.9 >= vmMips
//				&& h.getRamProvisioner().isSuitableForVm(vm, vm.getCurrentRequestedRam())
//                && h.getBwProvisioner().isSuitableForVm(vm, vm.getCurrentRequestedBw())
        );
	}
	...
	/**
	 *
	 * @param nonSelectedPheromones
	 * @param nonSelectedHeuristics
	 * @param notYetSelected
	 * @return
	 */
...
//				pheromoneList.get(i).set(j,((1- pheromoneDecay)*pheromoneList.get(i).get(j)+ pheromoneDecay * initialPheromone));
	...
	/**
	 *	Attractiveness, increases probability of the vms' selection when it is high
	 *	Inverse ratio with its hosts utilization
	 *	Returns complement value to its hosts utilization
	 * @param vm
	 * @return
	 */
	public double heuristicCalculationVm(Vm vm){
		return 1-(vm.getHost().getTotalMips()-vm.getHost().getAvailableMips())/vm.getHost().getTotalMips();
	}

	/**
	 * Attractiveness, increases probability of the hosts' selection when it is high
	 * Returns its utilization
	 * @param host
	 * @return
	 */
	...
	/**
	 *
	 * @param vmListWithoutNullHosts
	 * @param antSelectedVm
	 * @param hostSelectionResult
	 * @param antHostList
	 * @param antHeuristicsVM
	 * @param antHeuristicsHost
	 */
	public void heuristicUpdate(List<Vm> vmListWithoutNullHosts,
								int antSelectedVm,
								int hostSelectionResult,
								antHost antHostList,
		...
		int sourceHostIndex = hostList.indexOf(vmListWithoutNullHosts.get(antSelectedVm).getHost());

		double newTargetHostUtilization = antHostList.utilVal.get(hostSelectionResult)/hostList.get(hostSelectionResult).getTotalMips();
		double newSourceHostUtilization = antHostList.utilVal.get(sourceHostIndex)/hostList.get(sourceHostIndex).getTotalMips();

		antHeuristicsVM.set(antSelectedVm,vmHeuristicUpdate);
		antHeuristicsHost.set(hostSelectionResult,newTargetHostUtilization);
		antHeuristicsHost.set(sourceHostIndex,newSourceHostUtilization);
	}
	...
	public List<Double> heuristicInitializationVm(List<? extends Vm> vmList) {
		...
		double VMHeuristicAverage = sum/vmList.size();
		for(int i=0; i<vmList.size(); i++) {
	...
	public List<Double> heuristicInitializationHost(List<Host> hostList) {
		...
		double hostHeuristicAverage = sum/hostList.size();
		for(int i=0; i<hostList.size(); i++) {
	...
	// TODO instead of first fit, best fit algorithm should be implemented
	/**
