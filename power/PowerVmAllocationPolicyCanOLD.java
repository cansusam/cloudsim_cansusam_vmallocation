 ...
 * @author canssm
 */
public class PowerVmAllocationPolicyCanOLD extends PowerVmAllocationPolicyAbstract {
	...
	double initialPheromoneLevelVmAssign = canConstants.GAPACO_OLD.initialPheromoneLevelVmAssign;
	double initialPheromoneLevelHostSelect = canConstants.GAPACO_OLD.initialPheromoneLevelHostSelect;
	double relativeImportanceOfPMsToSleep = canConstants.GAPACO_OLD.relativeImportanceOfPMsToSleep;
	double relativeImportanceOfHeuristic = canConstants.GAPACO_OLD.relativeImportanceOfHeuristic;
	double relativeImportancePheromone = canConstants.GAPACO_OLD.relativeImportancePheromone;

	double iterationLimit = canConstants.GAPACO_OLD.iterationLimit;
	int antNumber = canConstants.GAPACO_OLD.antNumber;
	double parameterQ0 = canConstants.GAPACO_OLD.parameterQ0;
	double pheromoneDecayLocal = canConstants.GAPACO_OLD.pheromoneDecayLocal;
	double pheromoneDecayGlobal = canConstants.GAPACO_OLD.pheromoneDecayGlobal;
	 ...
	 * @param list the list
	 */
	public PowerVmAllocationPolicyCanOLD(List<? extends Host> list) {
		super(list);
	...
	public List<Map<String, Object>> optimizeAllocation(List<? extends Vm> vmList) {
		...
		for(int i=0; i<hostList.size(); i++) {
			...
			if(totalRequestedMips == hostList.get(i).getTotalMips())
			    sleepingHosts.add(hostList.get(i));
		}
		...
		while(iterationLimit > iteration){
			...
			for(int k=0; k<antNumber; k++){ // TODO antnumber <-> vmList.size()
	...
	public int selectNewHost(Vm selectedVm,
		...
		double destinationUtilization = theAntHost.utilVal.get(destinationHost) + vmMips;
